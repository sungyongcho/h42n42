...
let subdivide (qt: quadtree) =
  ...
  let w = qt.boundary.w in
  let h = qt.boundary.h in
  let half_w = w /. 2. in
  let half_h = h /. 2. in

  (* Define quadrant boundaries *)
  let nw_rect = { x = x -. (half_w /. 2.); y = y -. (half_h /. 2.); w = half_w; h = half_h } in
  let ne_rect = { x = x +. (half_w /. 2.); y = y -. (half_h /. 2.); w = half_w; h = half_h } in
  let sw_rect = { x = x -. (half_w /. 2.); y = y +. (half_h /. 2.); w = half_w; h = half_h } in
  let se_rect = { x = x +. (half_w /. 2.); y = y +. (half_h /. 2.); w = half_w; h = half_h } in
